...
def _svg_to_png(
    infile, outfile, pathcolour=_DEFAULT_PATHCOLOUR, background=_DEFAULT_BACKGROUND
):
    """Converts SVG file to PNG

    Assumes single colour SVG
    """
    drawing = svglib.svg2rlg(
        infile,
        color_converter=lambda colour_in: reportlab.lib.colors.HexColor(
            pathcolour.string
        ),
    )
    reportlab.graphics.renderPM.drawToFile(
        drawing, outfile, fmt="PNG", bg=background.integer
    )


def _svg_to_photoimage(
    infile, pathcolour=_DEFAULT_PATHCOLOUR, background=_DEFAULT_BACKGROUND
):  # , size):
...
class _Icons:
    ...
    def get(self, key, pathcolour=_DEFAULT_PATHCOLOUR, background=_DEFAULT_BACKGROUND):
        ...
        if hexcolour not in self._images[key]:
            ...
            self._images[key][hexcolour] = PIL.ImageTk.PhotoImage(
                _svg_to_photoimage(
                    filepath, pathcolour=pathcolour, background=background
                )
            )
