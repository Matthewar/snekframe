    ...
    with PERSISTENT_SESSION() as persistent_session, RUNTIME_SESSION() as runtime_session:
        ...
        for direntry in os.scandir(os.path.join(FILES_LOCATION, PHOTOS_LOCATION)):
            if direntry.is_dir():
                ...
                logging.warning("Found potential photo without album '%s'", direntry.name)
            else:
                logging.error("Found unknown file '%s'", direntry.name)

        num_photos = 0
        for album in albums:
            for direntry in os.scandir(os.path.join(FILES_LOCATION, PHOTOS_LOCATION, album)):
                if direntry.is_dir():
                    logging.warning("Ignoring directory '%s' in album '%s'", direntry.name, album)
                elif direntry.is_file():
                    if is_file_image(direntry.path):
                        ...
                        if found_image is None:
                            ...
                            logging.info("Rediscovered image '%s' in album '%s'", direntry.name, album)
                else:
                    logging.error("Found unknown file '%s' in album '%s'", direntry.name, album)

        persistent_session.commit()

        lost_files = runtime_session.execute(select(ExistingFiles.photo_path).where(ExistingFiles.found == True))
