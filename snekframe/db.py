...
class CurrentDisplay(PersistentBase):
    ...
    album: Mapped[Optional[str]] = mapped_column(
        String(MAX_FILENAME_SIZE), nullable=True
    )
...
class Settings(PersistentBase):
    ...
    # TODO:
    sleep_start_time: Mapped[Optional[datetime.time]] = mapped_column(
        Time(), nullable=True, insert_default=None
    )
    sleep_end_time: Mapped[Optional[datetime.time]] = mapped_column(
        Time(), nullable=True, insert_default=None
    )
...
PERSISTENT_ENGINE = create_engine(
    "sqlite:///{}".format(os.path.join(params.FILES_LOCATION, params.DATABASE_NAME))
)
